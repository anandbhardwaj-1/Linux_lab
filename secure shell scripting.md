...
ssh anand@10.0.2.15
```
**this command initiates an encrypted connection to the remote host as user ```anand```
...
```bash
scp ~/documents/report.pdf anand@10.0.2.15:/home/anand/
```
This:
...
```bash
scp anand@10.0.2.15:/home/anand/log.txt ~/downloads/
```
 ...
 *prompts a password (or uses your SSH key) ,then drops you into the sftp prompt:
 ...
 ```bash
 sftp> get /home/anand/report.txt ~/Documents/
```
...
```bash
sftp> get -r /home/anand/Projects ~/Backups/
```
or 
```bash
sftp> put -r ~/Projects /home/anand/
```
...
```bash
sftp anand@192.168.1.20:/home/anand/report.txt .
```
Or:
```bash
sftp -b batch_commands.txt anand@192.168.1.20
```
